    ...
    public void onClickView(View view, int id) {
        switch (id) {
            case R.id.send_location:
                if (mMsg != null) {
                    LocationProvider.getInstance().getLastLocationCallback().onSuccess(mMsg);
                    LocationProvider.getInstance().setLastLocationCallback(null);
                    finish();
                } else {
                    LocationProvider.getInstance().getLastLocationCallback()
                            .onFailure("定位失败");
                }
                break;
            default:
...
//        aMap.setMapType(AMap.MAP_TYPE_NORMAL);
    ...
    protected void onDestroy() {
        if (LocationProvider.getInstance().getLastLocationCallback() != null)
            LocationProvider.getInstance().getLastLocationCallback().onFailure("失败");
        LocationProvider.getInstance().setLastLocationCallback(null);
        super.onDestroy();
        mapView.onDestroy();
    }
