 ...
 * 发布轻小说
 */
public class NoStoryActivity extends AbsFragmentActivity implements PbTopBar.MytopbarClick,IViewModelInterface {
    ...
    public static final int GET_SERIALIZE_ENTITY=111;

    PbTopBar mytopbar;
    ImageView select_image;
    RadioGroup radio_serial,radio_type;
    EditText edit_dec,edit_title,edit_author;
    TextView edit_create_time,iv_arrow_serialize;

    public ArrayList<String> imgs=new ArrayList<>();
    String type,is_serial,time,author,serialid="";
    ...
    public int getLayoutId() {
        return R.layout.activity_release4;
    }
    ...
    public void initView() {
        ...
        radio_serial=(RadioGroup) findViewById(R.id.radio_serial);
        iv_arrow_serialize= (TextView) findViewById(R.id.iv_arrow_serialize);
        ...
        iv_arrow_serialize.setOnClickListener(this);
    }
    ...
    public void initData() {
            radio_type.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(RadioGroup group, int checkedId) {
                    ...
                    RadioButton rb = (RadioButton) NoStoryActivity.this.findViewById(radioButtonId);
                    type =  rb.getText().toString();
                  //  Toast.makeText(NoStoryActivity.this, rb.getText(), Toast.LENGTH_SHORT).show();
                }
            });
        radio_serial.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                //获取变更后的选中项的ID
                int radioButtonId = group.getCheckedRadioButtonId();
                //根据ID获取RadioButton的实例
                RadioButton rb = (RadioButton) NoStoryActivity.this.findViewById(radioButtonId);
                //is_serial= (String) rb.getText();
                if (rb.getText().equals("连载")) {
                    iv_arrow_serialize.setVisibility(View.VISIBLE);
                    edit_title.setEnabled(false);
                    is_serial = "1";
                    //serialid="xx";
                }
                else if(rb.getText().equals("短篇"))
                {
                    iv_arrow_serialize.setVisibility(View.GONE);
    ...
    public void onClickView(View view, int id) {
        switch (id)
        {
            ...
            {
                //封面选择
                Intent intent = new Intent(NoStoryActivity.this, SelectAlbumActivity.class);
            ...
            case R.id.iv_arrow_serialize:
                ChoseSerializeActivity.newInstance(this,entity,GET_SERIALIZE_ENTITY);
                break;
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == PICK_UP_IMAGE && null != data) {
            imgs = (ArrayList<String>) data.getSerializableExtra(SelectAlbumActivity.INTENT_SELECTED_PICTURE);
//            for(int position=0;position<imgs.size();position++) {
            UIL.load(select_image, "file://" + imgs.get(0));
            return;
        }
        if(requestCode==GET_SERIALIZE_ENTITY&&resultCode==RESULT_OK)
        {

            SubjectSerialEntity getentity=data.getParcelableExtra("entity1");
            iv_arrow_serialize.setText("");
            serialid=getentity.getId();
            edit_title.setText(getentity.getName());
        }
    }
    ...
    public void rightTexClick() {
        ...
        String user_id,dec,author,title;
        ...
        if(imgs.size()>0&&!TextUtils.isEmpty(dec)&&!TextUtils.isEmpty(title)&&!TextUtils.isEmpty(author)) {
            File imageFile = new File(imgs.get(0));
            files.add(imageFile);
            addSpecialModel.start("", title, user_id, type, entity.getId(), "", author, "", "", is_serial, files,dec,"0",serialid);
            showLoadingDialog();
        }
        else {
            Toast.makeText(this, getString(R.string.have_null), Toast.LENGTH_SHORT).show();
        }
    }
