...
//        	processCustomMessage(context, bundle);
            ...
            Intent i = new Intent(Constants.ACTION_PUSH_MSG);
            context.sendBroadcast(i);

        } else if (JPushInterface.ACTION_NOTIFICATION_OPENED.equals(intent.getAction())) {
            Log.d(TAG, "[MyReceiver] 用户点击打开了通知");

            if (BaseApp.isRunningForeground()) {
                Log.d(TAG, "[MyReceiver] isRunningForeground 已经打开了");
                BaseApp.notifyMessage();
            } else {
                //打开自定义的Activity
                Intent i = new Intent(context, NaviActivity.class);
                i.putExtra(Constants.ACTION_PUSH_MSG, 1);
