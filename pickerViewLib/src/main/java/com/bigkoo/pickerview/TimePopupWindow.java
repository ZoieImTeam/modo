...
public class TimePopupWindow extends PopupWindow implements OnClickListener {
	...
	public boolean ismCheckForAge() {
		return mCheckForAge;
	}

	public void setmCheckForAge(boolean mCheckForAge) {
		this.mCheckForAge = mCheckForAge;
	}

	private boolean mCheckForAge;
	...
	public TimePopupWindow(Context context, Type type) {
		...
		wheelTime.mScreenHeight = screenInfo.getHeight();
	...
	public void showAtLocation(View parent, int gravity, int x, int y, Date date) {
		...
		if (mCheckForAge){
			...
			wheelTime.setPicker(year, month, day, hours, minute, true);
		}
		update();
			...
			if (timeSelectListener != null) {
				try {
					...
					if (mCheckForAge){
						...
						date = WheelTime.dateNewFormat.parse(wheelTime.getNewTime());
					}
					if (date.after(new Date()) ){
						if(mCheckForAge){
							Toast.makeText(mContext, R.string.time_not_before_now ,Toast.LENGTH_SHORT).show();
						}else{
